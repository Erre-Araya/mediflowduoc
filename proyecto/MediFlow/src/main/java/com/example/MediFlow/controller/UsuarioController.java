...
import com.example.MediFlow.dto.LoginRequest;
...
public class UsuarioController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest request) {
        try {
            ...
            System.out.println("INPUT correo: [" + request.getCorreo() + "]");
            System.out.println("INPUT password: [" + request.getPassword() + "]");

            List<Usuario> usuarios = usuarioRepository.findAll();

            for (Usuario u : usuarios) {
                System.out.println("BD correo: [" + u.getCorreo() + "]");
                System.out.println("BD password: [" + u.getPassword() + "]");
                System.out.println("----");
            }

            Usuario usuario = usuarioRepository
                .findByCorreo(request.getCorreo()).orElseThrow(() -> new RuntimeException("Usuario no encontrado"));
        ...
        if (rolEnum == Rol.PACIENTE &&
            !rolUsuario.equalsIgnoreCase("PROFESIONAL") &&
        ...
        public ResponseEntity<?> cambiarPasswordObligatorio(
            ...
            try {
                ...
                UsuarioDTO usuario = usuarioService.cambiarPasswordObligatorio(id, password);
        ...
        @PatchMapping("/{id}/password/perfil")
        public ResponseEntity<?> cambiarPasswordPerfil(
                @PathVariable Long id,
                @RequestBody Map<String, Object> request
        ) {
            try {
                String passwordActual = request.get("passwordActual").toString();
                String nuevaPassword = request.get("nuevaPassword").toString();

                UsuarioDTO usuario = usuarioService.cambiarPasswordPerfil(
                        id,
                        passwordActual,
                        nuevaPassword
                );
        ...
        @PostMapping("/admin/paciente")
        public ResponseEntity<?> crearPacienteAdmin(
                @RequestBody Map<String, Object> request,
                @RequestHeader("rol") String rolUsuario
        ) {
            try {
                if (!"ADMIN".equalsIgnoreCase(rolUsuario)) {
                    return ResponseEntity.status(403)
                            .body(Map.of("error", "Solo admin puede crear pacientes"));
                }

                UsuarioDTO creado = usuarioService.crearPacienteAdmin(request);
                return ResponseEntity.status(HttpStatus.CREATED).body(creado);
